    ...
    <div className="flex-1 flex flex-col min-h-full lg:h-full bg-zinc-900 text-zinc-100 overflow-y-auto lg:overflow-hidden">
      ...
      <div className="px-4 sm:px-8 py-4 sm:py-5 border-b border-zinc-800 bg-zinc-950 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shrink-0">
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-0.5">Add delivery stops, avoid road hazards, and find the fastest route for your trucks.</p>
        </div>
        <button 
        ...
        >
          ...
          <span>Try Lagos Sample Route</span>
        </button>
      </div>
      ...
      <div className="flex-1 flex flex-col lg:flex-row min-h-0 overflow-y-auto lg:overflow-hidden">
        ...
        <div className="w-full lg:w-96 flex flex-col border-b lg:border-b-0 lg:border-r border-zinc-800 bg-zinc-950/20 overflow-y-auto shrink-0 p-4 sm:p-6 space-y-6">
          ...
          <div className="space-y-3">
            <label className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Choose What to Add</label>
            ...
            {clickMode === 'stop' && (
              <div className="p-3 bg-zinc-900/60 border border-zinc-800 rounded-xl space-y-2.5">
                <input 
                  ...
                  placeholder="Stop Name (e.g. Yaba Store)"
                ...
                <div className="flex items-center justify-between gap-4">
                  <span className="text-[10px] font-semibold text-zinc-400 uppercase tracking-wider">Package Weight (kg)</span>
                  <input 
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <label className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Avoid Road Obstacles</label>
              <input 
            ...
            <p className="text-[11px] text-zinc-500">Automatically navigate around police inspection stops (+15m), potholes and bad roads, and flooded areas.</p>
          </div>

          {/* Vehicles selection */}
          <div className="space-y-3">
            <label className="text-xs font-bold text-zinc-400 uppercase tracking-widest flex items-center justify-between">
              <span>Choose Available Trucks</span>
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto pr-1">
              {vehicles.map((v) => (
                ...
                >
                  ...
                  <span className="text-xs font-medium">{v.capacity_kg}kg max</span>
                </div>
              ))}
            </div>
          </div>
            ...
            >
              {isOptimizing ? (
                <>
                  ...
                  <span>Finding Fastest Delivery Routes...</span>
                </>
              ) : (
                <>
                  <Play className="w-4 h-4 fill-zinc-950" />
                  <span>Find Best Delivery Routes</span>
                </>
              )}
            </button>
            {optimizerError && (
          ...
          <div className="flex-1 flex flex-col min-h-0 space-y-3">
            <div className="flex items-center justify-between">
              <label className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Delivery Stops ({stops.length})</label>
              {stops.length > 0 && (
                <button 
            ...
            {stops.length === 0 ? (
              <div className="flex-1 border border-dashed border-zinc-800 rounded-xl flex flex-col items-center justify-center p-4 text-center">
                ...
                <p className="text-[11px] text-zinc-500 leading-normal">No delivery stops added yet. Click anywhere on the map to add delivery addresses.</p>
              </div>
            ) : (
          ...
          {optimizationResult && (
            <div className="h-64 border-t border-zinc-800 bg-zinc-950/90 backdrop-blur-md overflow-y-auto p-6 space-y-4 shrink-0 z-[1000]">
              <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
                <div>
                  <h3 className="text-sm font-bold text-white uppercase tracking-wider">Best Delivery Route Plan</h3>
                  <p className="text-xs text-zinc-400">Total estimated driving time: {Math.round(optimizationResult.total_time_seconds / 60)} minutes.</p>
                </div>
                <div className="flex items-center gap-2 text-xs font-semibold text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-3 py-1 rounded-full">
                  ...
                  <span>Fastest Routes Calculated!</span>
                </div>
              </div>
                ...
                {optimizationResult.routes.map((route: any, idx: number) => {
                  ...
                  return (
                    <div key={idx} className="p-4 bg-zinc-900 border border-zinc-800 rounded-xl flex flex-col justify-between space-y-3">
                      <div>
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-bold text-zinc-200">Trip #{idx + 1}</span>
                          <span 
                        ...
                        <div className="mt-2 space-y-1 text-xs">
                          <div className="flex justify-between">
                            <span className="text-zinc-500">Delivery Truck:</span>
                            <span className="font-semibold text-zinc-300">{vehicle?.plate_number || `Truck ${route.vehicle_id + 1}`}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-zinc-500">Stops on this trip:</span>
                            <span className="font-semibold text-zinc-300">{Math.max(0, route.stops.length - 2)} customer stops</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-zinc-500">Cargo Weight:</span>
                          ...
                          <div className="flex justify-between">
                            <span className="text-zinc-500">Est. Driving Time:</span>
